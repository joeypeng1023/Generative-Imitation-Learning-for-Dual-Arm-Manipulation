...
def collect_human_trajectory(env, device, arm, max_fr, goal_update_mode):
    ...
    # Keep track of current actions for all arms to prevent auto-reset
    all_current_actions = []
    for robot in env.robots:
        robot_actions = {}
        for robot_arm in robot.arms:
            robot_actions[f"{robot_arm}_delta"] = np.zeros(3)  # Position delta
            robot_actions[f"{robot_arm}_abs"] = np.zeros(3)    # Absolute position
            if robot.gripper[robot_arm].dof > 0:
                robot_actions[f"{robot_arm}_gripper"] = np.zeros(robot.gripper[robot_arm].dof)
            else:
                robot_actions[f"{robot_arm}_gripper"] = np.array([])
        all_current_actions.append(robot_actions)
    ...
    while True:
        ...
        # For inactive robots, maintain their current state to prevent auto-reset
        env_action = []
        for i, robot in enumerate(env.robots):
            if i == device.active_robot:
                # Update active robot with new action
                env_action.append(active_robot.create_action_vector(action_dict))
                # Update gripper state tracking
                for gripper_ac in all_prev_gripper_actions[device.active_robot]:
                    all_prev_gripper_actions[device.active_robot][gripper_ac] = action_dict[gripper_ac]
            else:
                # Keep inactive robots in their current state
                env_action.append(robot.create_action_vector(all_current_actions[i]))
...
if __name__ == "__main__":
    ...
    # Create environment with fixed camera to prevent rendering issues
    env = suite.make(
        **config,
        has_renderer=True,
